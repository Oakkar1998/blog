...
<table class="table table-hover table-warning">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <img src="{{ asset('storage/'.$article->image) }}" class="border border-dark rounded-sm" alt=""
                    width="100" height="auto">
            </td>
            <td class="align-middle">{{ Str::limit($article->title, 20, '...') }}
            </td>
            <td class="align-middle">{{ $article->author }}</td>
            <td class="align-middle">reactions</td>
            <td class="align-middle">{{ $article->created_at->format('M d, Y') }}</td>

            <td class="align-middle">
                <a href="{{ route('creator.viewArticlePage',$article->id) }}" class="btn btn-sm btn-warning">view</a>
                <a href="{{ route('creator.editArticlePage',$article->id) }}" class="btn btn-sm btn-dark">Edit</a>
                <!-- Delete Button -->
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                Are you sure you want to delete this article: <strong>{{ $article->title }}</strong>?
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>

                                <form action="{{ route('article.deleteArticle', $article->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
